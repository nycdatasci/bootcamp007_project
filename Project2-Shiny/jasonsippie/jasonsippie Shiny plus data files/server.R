...
# County Plot
  ...
  output$detPlot <- renderPlotly( {
    ...
    titleList = c(num_emp="Num Employees",
                  tot_payroll="Total Payroll",
                  num_est="Num establishments")
    thisTitle = titleList[[input$metric]]
    
    print(thisTitle)
    if (thisTitle != "Num Employees") {
      ttl = paste(thisTitle,"000's")
    } else {
      ttl = thisTitle
    }
