...
2. select bike availability count:
...
3. select radius between 1 - 5 miles
4. from and to address - produce avg ride time based on trip duration from citibike data file
5. select ride time: 30 / 45 min
5. select peak time view - avg bike count based on prior month data

Would be nice:
1. Show available bikes and docking counts within radius of the from / to address
...
#cb_json <- fromJSON("http://citibikenyc.com/stations/json")
cb_json <- fromJSON(cb_url)
