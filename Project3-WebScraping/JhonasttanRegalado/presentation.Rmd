---
title: "Web Scraping Project: Visualize and Analyze Starbucks Store Amenities"
...
* Scrapy for web scraping stores and their amenities via Starbucks Store Locator website
...
* Regex (regular expressoins) for creating amenities features - used as filter criteria in CARTO map layers
...
p1 <- ggplot(amenSums,aes(reorder(amenities,amenSums),amenSums)) + #, label = amenSums$amenSums
  ...
  ggtitle("Starbucks Store Amenities") +
...
##### MCA results shows amenitiesCD and MX are grouped with the five most common amenities.
...
### Follow ups:
  ...
  1. Create a Map Layer for Shiny Citibike Analysis app, allowing users to locate Starbucks stores based on amenities and proximity
