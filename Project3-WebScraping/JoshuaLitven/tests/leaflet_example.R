...
load("Courses/nyc_data_science_academy/projects/web_scraping/tests/nwt_locations.RData")
...
ui <- bootstrapPage(
  ...
  absolutePanel(top=10, right=10,
    actionButton("random_button", "Random Community", class="btn-block")
  )
)
...
server <- function(input, output, session) {
  ...
  output$Map <- renderLeaflet({
    leaflet() %>% setView(lon, lat, 4) %>% addTiles() %>%
      addCircleMarkers(data=locs, radius=6, color="black", stroke=FALSE, fillOpacity=0.5, 
                       group="locations", layerId = ~loc)
  })
  ...
  observeEvent(input$random_button, {
    ...
    p <- sample_n(locs, 1)
    proxy = leafletProxy("Map")
    proxy %>% setView(lng=p$lon, lat=p$lat, input$Map_zoom) %>% acm_defaults(p$lon, p$lat)
  })
