...
![Caption](assets/img/marriage.small.jpg) -Dad & Mom: Marriage still important, but ..
```{r}
ggplot(asian_adult[asian_adult$workclass=="PRIV",], aes(x=marital_status)) + 
  geom_bar(aes(fill=marital_status)) + facet_grid(class ~ sex) +
  xlab("marital status") + 
  theme_economist() + scale_fill_economist() +
  ggtitle("Asian adult in private sector - People never married also earn more.")
```

## A few thoughts

I think the asian adult sample in 1994 *Adult* dataset (895) may be too small.  I may need to explore samples for more than a single year.

As an asian myself, I listen and obey my parents as they always want the best for me.  Most things change as time evolves; thus, it is reasonable to say very few things hold true forever.  But, parents' care to their children never change.  
