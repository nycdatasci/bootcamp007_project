...
p <- ggplot(typhoon.all, aes(Longitude, Latitude, colour = Name)) + 
  ...
  geom_polygon(aes(x, y, colour = "navy"), jpn) + # draws the map? how?
  coord_map(xlim = long.lim, ylim = lat.lim) +  # set coordinates to mercator
  scale_size_continuous(range = c(1, 10), trans = "reverse") #scale point
...
typhoon.2016.g1 <- jpn.map +
  geom_point(data = typhoon.all, aes(Longitude, Latitude, size = Pressure, col = Name), 
...
distance.g <- ggplot(typhoon.distances, aes(x=DistanceTraveled)) +
  ...
  xlab("DistanceTraveled (km)") + ggtitle("Typhoon Distance Traveled")
