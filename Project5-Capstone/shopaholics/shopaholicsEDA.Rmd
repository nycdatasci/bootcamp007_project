...
Load data into dataframe and capture geospatial locations
```{r}
...
locations <- users %>% group_by(Location) %>% count()
...
for (i in seq_along(users$Location)) {
  locationGeocode <- tryCatch(
    userLocationGeoSpatial(users$Location[i]), 
    error=function(e) NULL
    )
  if (length(locationGeocode) > 1) {
    users$lon[i] <- locationGeocode[[1]]
    users$lat[i] <- locationGeocode[[2]]
  }
...
leaflet(data = users[1:30,c("Location","lon","lat")]) %>% 
  addTiles(urlTemplate = tile_layer) %>% 
  addMarkers(popup = users$Location) # %>%
