...
```{r}
#cleaned data
users <- read.csv("./usersCleanedWithNAs.csv", sep = ";", header = TRUE, stringsAsFactor=FALSE, quote="\"", encoding = "UTF-8") 
...
for (i in seq_along(locationsGeoCoded$Location)) {
  loc <- str_split(string = locationsGeoCoded$Location[i], pattern = ",")
  locationsGeoCoded$city[i] <- loc[[1]][1]
  locationsGeoCoded$state[i] <- loc[[1]][2]
  locationsGeoCoded$country[i] <- loc[[1]][3]
  
}

write.csv(locationsGeoCoded,file = "./locationsExpanded.csv")
...
isbn1k <- read.csv("./ratedISBNunique.csv",header = TRUE, stringsAsFactors = FALSE)
...
for (i in c(75183:85000)) {
  ...
  if (length(bookJson) > 2) {
    ...
    tryCatch.W.E( booksCatalogue <- rbind(booksCatalogue, jasonIsbnParse(bookJson)) )
  }
  Sys.sleep(1/2)
  if (i %in% c(95000, 115000, 135,000, 149842)) {
    write.csv(booksCatalogue, file = "./googleBooksLookup2.csv", append = TRUE)
    booksCatalogue <- data.frame()
